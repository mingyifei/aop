...
/*        try {
            Reader<byte[]> reader = readerMap.computeIfAbsent(queue.toString(), k -> {
                try {
                    return connection.getPulsarService().getClient()
                            .newReader()
                            .startMessageId(MessageId.latest)
                            .topic(getTopicName(PersistentQueue.TOPIC_PREFIX, queue.toString()))
                            .create();
                } catch (Exception e) {
                    throw new AoPServiceRuntimeException.ReaderCreationRuntimeException(e);
                }
            });
            ...
            connection.getAmqpOutputConverter().writeGetOk(
                    MessageConvertUtils.messageToAmqpBody(message), getChannelId(), false,
                    deliveryIndex, 0);
            if (noAck) {
                ack(queue, message.getMessageId());
            } else {
                getUnacknowledgedMessageMap().add(
                        deliveryIndex, PositionImpl.get(messageId.getLedgerId(), messageId.getEntryId()),
                        new UnacknowledgedMessageMap.MessageProcessor() {
                            @Override
                            public void messageAck(Position position) {
                                ack(queue, new MessageIdImpl(position.getLedgerId(), position.getEntryId(), -1));
                            }
                            ...
                            public void discardMessage(List<PositionImpl> positions) {
                                positions.forEach(position -> ack(queue,
    ...
    private void ack(AMQShortString queue, MessageId messageId) {
        try {
            connection.getPulsarService().getClient().newConsumer()
                    .topic(getTopicName(PersistentQueue.TOPIC_PREFIX, queue.toString()))
                    ...
                    .subscribeAsync()
                    .thenAccept(consumer -> consumer.acknowledgeAsync(messageId)
                            .thenAccept(__ -> consumer.closeAsync()));
